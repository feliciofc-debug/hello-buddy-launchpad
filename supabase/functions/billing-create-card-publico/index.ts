...
serve(async (req) => {
  ...
  try {
    ...
    const preference = {
      items: [{
        title: 'Mensalidade AMZ Ofertas PRO',
      ...
      payer: {
        email,
        name: first_name || (document_type === 'cnpj' ? company_name : 'Cliente') || 'Cliente',
        surname: last_name || 'AMZ Ofertas',
        identification: {
      ...
      back_urls: {
        success: `${origin}/pagar/${subscription_id}?status=success`,
        failure: `${origin}/pagar/${subscription_id}?status=failure`,
        pending: `${origin}/pagar/${subscription_id}?status=pending`,
      },
    ...
    if (!mpResp.ok) {
      console.error('MP erro cartão público:', data);
      return bad(data?.message || 'Erro ao criar checkout de cartão', 400);
    }
    ...
    console.error('billing-create-card-publico erro:', e);
