...
serve(async (req) => {
  ...
  try {
    ...
    // Server-side: amount vem do banco, NUNCA do cliente
    ...
    const payload = {
      ...
      payer: {
        email,
        first_name: first_name || (document_type === 'cnpj' ? company_name : 'Cliente') || 'Cliente',
        last_name: last_name || 'AMZ Ofertas',
        identification: {
    ...
    if (!mpResp.ok) {
      console.error('MP erro PIX público:', data);
      let msg = 'Erro ao gerar PIX';
      if (data?.cause?.[0]?.code === '2067') msg = 'CPF inválido. Verifique o número.';
      else if (data?.cause?.[0]?.code === '106') msg = 'Email inválido.';
      else if (data?.cause?.[0]?.description) msg = data.cause[0].description;
    ...
    console.error('billing-create-pix-publico erro:', e);
