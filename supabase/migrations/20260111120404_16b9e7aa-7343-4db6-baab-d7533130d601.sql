-- Adicionar coluna para controle de rotação por marketplace
ALTER TABLE programacao_envio_afiliado 
ADD COLUMN IF NOT EXISTS marketplaces_ativos TEXT[] DEFAULT ARRAY['Amazon', 'Shopee', 'Magazine Luiza', 'Mercado Livre'],
ADD COLUMN IF NOT EXISTS ultimo_marketplace_enviado TEXT DEFAULT NULL,
ADD COLUMN IF NOT EXISTS produtos_no_marketplace_atual INTEGER DEFAULT 0;

-- Atualizar função para rotacionar entre marketplaces
...
BEGIN
  ...
  IF v_contador >= 3 OR v_programacao.ultimo_marketplace_enviado IS NULL THEN
    ...
    v_contador := 0;
  ELSE
    v_proximo_marketplace := v_programacao.ultimo_marketplace_enviado;
  END IF;
  
  -- Buscar produto do marketplace selecionado que não foi enviado recentemente
  ...
  WHERE p.user_id = v_programacao.user_id
    ...
    AND p.id != COALESCE(v_programacao.ultimo_produto_id, '00000000-0000-0000-0000-000000000000')
    AND (v_programacao.categorias IS NULL OR array_length(v_programacao.categorias, 1) = 0 
         OR p.categoria = ANY(v_programacao.categorias))
  ORDER BY RANDOM()
  LIMIT 1;
  
  -- Se não encontrou produto no marketplace atual, tentar próximo
  IF v_produto IS NULL THEN
    -- Tentar outros marketplaces
    FOR i IN 1..array_length(v_marketplaces_disponiveis, 1) LOOP
