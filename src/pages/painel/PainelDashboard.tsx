...
export default function PainelDashboard() {
  ...
  const loadClientes = async () => {
    const { data: profiles } = await supabase
      .from("profiles")
      .select("id, nome, nome_fantasia, whatsapp, tipo")
    ...
    const { data: customers } = await supabase
      ...
      .select("id, email, platform_login");
    ...
    const clientesList: ClienteInfo[] = profiles.map((p: any) => {
      const customer = customers?.find(
        (c: any) => c.platform_login === p.id || c.email === p.whatsapp
      );
      const sub = customer
        ? subs?.find((s: any) => s.customer_id === customer.id)
        : null;
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
          ...
          <Card className="bg-slate-800 border-slate-700">
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Funções Online</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-slate-800 border-slate-700">
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Funções Offline</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-slate-800 border-slate-700">
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Com Falhas</p>
              </div>
            </CardContent>
          </Card>
        </div>
          ...
          <Card className="bg-slate-800 border-slate-700">
            ...
            <CardContent className="space-y-2 max-h-[400px] overflow-y-auto">
              {loading ? (
                ...
                filteredClientes.slice(0, 20).map((cliente, i) => (
                  <div
                    key={i}
                  ...
                  >
                    <div className="min-w-0">
                      <p className="text-sm text-white font-medium truncate">
                        {cliente.nome_fantasia || "Sem nome"}
                      </p>
        ...
        {/* Cliente selecionado - Detalhes */}
        {selectedCliente && (
          <Card className="bg-slate-800 border-slate-700">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-white text-base">
                  Detalhes: {selectedCliente.nome_fantasia || selectedCliente.email}
                </CardTitle>
                <Button
                  variant="ghost"
                  size="sm"
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="p-3 bg-slate-700/50 rounded-lg">
                  ...
                  <p className="text-sm">
                    {selectedCliente.whatsapp_connected ? (
                ...
                <div className="p-3 bg-slate-700/50 rounded-lg">
                  ...
                  <p className="text-sm">
                    {selectedCliente.subscription_status === "active" ? (
                      <span className="text-emerald-400">Ativa</span>
                    ) : (
                      <span className="text-amber-400">
                        {selectedCliente.subscription_status}
                      </span>
                    )}
                  </p>
                </div>
