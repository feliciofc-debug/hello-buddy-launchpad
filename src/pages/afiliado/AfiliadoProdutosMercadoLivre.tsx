  ...
  const loadProdutos = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select("*")
        .eq("user_id", user.id)
        .or("marketplace.ilike.%mercado%,marketplace.ilike.%meli%")
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Gerencie seus produtos de afiliado Mercado Livre
            </p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <Card className="max-w-lg mx-auto mt-12">
          <CardHeader className="text-center">
            ...
            <CardTitle>Nenhum produto Mercado Livre</CardTitle>
            <CardDescription>
              Use a extensão Chrome para importar produtos do Mercado Livre
            </CardDescription>
          </CardHeader>
          <CardContent className="text-center">
            <p className="text-sm text-muted-foreground mb-4">
              A extensão já captura o link de afiliado completo do Mercado Livre.
            </p>
          </CardContent>
        </Card>
      ) : (
          ...
          {produtos.map((produto) => (
            <Card key={produto.id} className="overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <CardContent className="p-3">
                ...
                <p className="text-xs text-muted-foreground mb-2">Mercado Livre</p>
